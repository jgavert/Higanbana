...
{
  return udBox(rotateZ(rotateY(p, sin(constants.time*0.1)*PII), cos(constants.time*0.1)*PII), b);
}
...
float3 getCameraRay(float3 posi, float3 dir, float2 pos) {
  ...
  float3 image_point = pos.x * float3(0, 0, 1) + pos.y*float3(0, 1, 0) + posi + dir;
...
{
  int2 iResolution = int2(constants.resx, constants.resy);
...
{ 
  ...
  int2 iResolution = int2(constants.resx, constants.resy);
  // fp /= float2(float(constants.resx), float(constants.resy));

  output[id] = float4(sin(constants.time)*0.2 + fp.x, cos(constants.time)*0.5 + fp.y, 0.f, 1.f);

  float3 position = float3(0, 0, 0); // iPos;
  float3 direction = float3(1, 0, 0); // iDir;
