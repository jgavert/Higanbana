...
namespace higanbana
{
  ...
  {    
    ...
    std::string ShaderStorage::sourcePathCombiner(std::string shaderPath, ShaderType type)
    {
      return shaderPath + "." + shaderFileType(type) + ".hlsl";
    }

    std::string ShaderStorage::binaryPathCombiner(std::string shaderPath, ShaderType type, uint3 tgs, std::vector<std::string> definitions)
    {
      ...
      return compiledPath + shaderBinType + binType + shaderPath + additionalBinInfo + "." + shaderFileType(type) + shaderExtension;
    }

    std::string shaderStubFile(ShaderCreateInfo info, std::string interfaceName)
    {
      std::string superSimple = "#include \"" + interfaceName + ".if.hlsl\"\n";
...
{ \
  ...
  float4 pos : SV_Position; \
    ...
    void ShaderStorage::ensureShaderSourceFilesExist(ShaderCreateInfo info)
    {
      ...
      auto shaderInterfacePath = info.desc.shaderName + ".if.hlsl";
      ...
      if (!m_fs.fileExists(shaderPath))
      {
        ...
        auto OnlyInterfaceName = info.desc.shaderName.substr(m_fs.mountPoint(info.desc.shaderName).size()+1);
        std::string superSimple = shaderStubFile(info, OnlyInterfaceName); 
