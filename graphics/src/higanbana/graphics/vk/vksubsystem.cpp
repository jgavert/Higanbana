  ...
  namespace backend
  {
    ...
    vector<GpuInfo> VulkanSubsystem::availableGpus(VendorID vendor)
    {
      ...
      for (devId = 0; devId < static_cast<int>(m_devices.size()); ++devId)
      {
        ...
        info.canVariableRate = false; // unimplemented;
        info.canRaytrace = false; // unimplemented;
    ...
    std::shared_ptr<backend::prototypes::DeviceImpl> VulkanSubsystem::createGpuDevice(FileSystem& fs, GpuInfo gpu)
    {
      ...
      std::shared_ptr<VulkanDevice> impl = std::make_shared<VulkanDevice>(dev, physDev, loader, fs, queueProperties, gpu, false, dev2prop.bufferDeviceAddress);
