...
namespace higanbana
{
  namespace backend
  {
    ...
    class UploadLinearAllocatorGPU
    {
      LinearAllocator allocator;
      UploadBlockGPU block;

    public:
      UploadLinearAllocatorGPU() {}
      UploadLinearAllocatorGPU(UploadBlockGPU block)
        : allocator(block.size())
        , block(block)
      {
      }

      UploadBlockGPU allocate(size_t bytes, size_t alignment)
      {
        auto offset = allocator.allocate(bytes, alignment);
        if (offset < 0)
          return UploadBlockGPU{ nullptr, 0, nullptr, nullptr, RangeBlock{} };

        UploadBlockGPU b = block;
        b.block.offset += offset;
        HIGAN_ASSERT(b.block.offset % alignment == 0, "hupsies");
        b.block.size = roundUpMultiplePowerOf2(bytes, alignment);
        HIGAN_ASSERT(b.block.size >= bytes, "hupsies v2");
    ...
    private:
      ...
      std::mutex m_allocatorLock;
    public:
      DX12ConstantsHeap() : allocator(1, 1), m_type(Type::OnlyCPU) {}
      DX12ConstantsHeap(ID3D12Device* device, unsigned memoryAmount, Type type)
        : allocator(memoryAmount, 16)
        , m_type(type)
      {
        {
      ...
      void release(UploadBlockGPU desc)
      {
        std::lock_guard guard(m_allocatorLock);
        allocator.free(desc.block);
      }
    ...
    {
      vector<UploadBlockGPU> uploadBlocks;
