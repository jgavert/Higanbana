...
namespace higanbana
{
  namespace backend
  {
    ...
    void DeviceGroupData::waitGpuIdle() {
      ...
      {
        vdev.device->waitTimeline(vdev.timelineGfx, vdev.gfxQueue);
        vdev.device->waitTimeline(vdev.timelineCompute, vdev.cptQueue);
        vdev.device->waitTimeline(vdev.timelineDma, vdev.dmaQueue);
    ...
    void setViewRange(const ShaderViewDescriptor& view, const ResourceDescriptor& desc, bool srv, ViewResourceHandle& handle) {
      ...
      handle.subresourceRange(mipOffset, mipLevels, sliceOffset, arraySize);
    }
    ...
    Texture DeviceGroupData::createTexture(ResourceDescriptor desc) {
      HIGAN_CPU_FUNCTION_SCOPE();
      auto handle = m_handles.allocateResource(ResourceType::Texture);
      handle.setMipCount(desc.desc.miplevels);
      ...
      //HIGAN_LOGi("created Texture: %zd: %s\n", handle.id, desc.desc.name.c_str());
    ...
    ShaderArguments DeviceGroupData::createShaderArguments(ShaderArgumentsDescriptor& binding) {
      ...
      for (auto& vdev : m_devices) // uh oh :D TODO: maybe not dynamic buffers for all gpus? close eyes for now
      {
        ...
        vdev.shaderArguments[handle] = ShaderArgumentVectors{ binding.bResources(), binding.bBindless() };
      }
    ...
    void DeviceGroupData::firstPassBarrierSolve(
      ...
      backend::AccessStage currentBoundType = backend::AccessStage::Graphics;
        ...
        while((*iter)->type != PacketType::EndOfPackets)
        {
          ...
          switch (header->type)
          {
            ...
            case PacketType::ResourceBindingGraphics:
            {
              ...
              auto stage = backend::AccessStage::Graphics;
              if (stage != currentBoundType)
              {
                boundSets[0] = {};
                boundSets[1] = {};
                boundSets[2] = {};
                boundSets[3] = {};
                currentBoundType = stage;
              }
              ...
              int i = 0;
              for (auto&& handle : packet.resourcesView())
              {
                if (boundSets[i] != handle)
                {
                  boundSets[i] = handle;
                  const auto& views = vdev.shaderArguments[handle];
                  auto addType = [&solver, &stage, &queue, &usedDrawIndex](ViewResourceHandle resource) {
                  ...
                  for (auto&& resource : views.bindless)
                  {
                    addType(resource);
                  }
                }
            ...
            {
              auto& packet = header->data<gfxpacket::ResourceBindingCompute>();
              auto stage = backend::AccessStage::Compute;
              if (stage != currentBoundType)
              {
                boundSets[0] = {};
                boundSets[1] = {};
              ...
              auto usedDrawIndex = drawIndex;
              HIGAN_ASSERT(!insideRenderpass, "not allowed");
              if (insideRenderpass)
              {
                usedDrawIndex = drawIndexBeginRenderpass;
              }
            ...
            case PacketType::UpdateTexture:
            {
              ...
              viewhandle.subresourceRange(packet.mip, 1, packet.slice, 1);
            ...
            case PacketType::BufferToTextureCopy:
            {
              ...
              dst.subresourceRange(packet.mip, 1, packet.slice, 1);
            ...
            case PacketType::PrepareForPresent:
            {
              ...
              viewhandle.subresourceRange(0, 1, 0, 1);
      ...
      if (!nodes.empty())
      {
        ...
        std::for_each(std::execution::par_unseq, std::begin(readyLists), std::end(readyLists), [&](backend::LiveCommandBuffer2& list) {
          ...
          std::for_each(std::execution::par_unseq, std::begin(list.listIDs), std::end(list.listIDs), [&](int id){
            ...
            {
              HIGAN_CPU_BRACKET("BarrierSolver reset");
              list.solver[id - offset]->reset(&vdev.m_bufferStates, &vdev.m_textureStates);
            }
    ...
    css::Task<std::shared_ptr<css::Task<void>>> DeviceGroupData::localPass(css::Task<std::shared_ptr<css::Task<void>>>* before, css::Task<void>* gcDone, std::optional<Swapchain> swapchain, vector<PreparedCommandlist>& lists, backend::LiveCommandBuffer2& liveList, int listID, int listIdBegin) {
      ...
      co_return std::make_shared<css::Task<void>>(finalPass((before != nullptr ? before->get().get() : nullptr), gcDone, swapchain, lists, liveList, listID, listIdBegin));
    }
