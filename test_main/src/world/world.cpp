...
void World::loadGLTFSceneCgltf(higanbana::Database<2048>& database, higanbana::FileSystem& fs, std::string dir)
{
  ...
  for (auto&& file : fs.recursiveList(dir, ".gltf"))
  {
    ...
    cgltf_result result = cgltf_parse(&options, static_cast<const void*>(fdata.data()), fsize, &data);
    if (result == cgltf_result_success)
    {
...

      {
        ...
        higanbana::unordered_map<std::string, higanbana::Id> mapToImages;
        {
          ...
          for (cgltf_texture& tex : higanbana::MemView(data->textures, data->textures_count))
          {
            cgltf_image* imagePtr = tex.image;
            if (imagePtr != nullptr && mapToImages.find(std::string(imagePtr->uri)) == mapToImages.end())
            {
              auto& image = *imagePtr;
              //HIGAN_LOGi("image: %s\n", image.uri);
              //HIGAN_ASSERT(image.buffer_view != nullptr, "WTF!");
